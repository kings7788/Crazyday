package org.iii.ee10087.itube._16_.Manager.controller;
...
@WebServlet("/manager/Managerlogin.do")
public class ManagerLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 無
		// 3. 檢查使用者輸入資料
		// 如果 account 欄位為空白，放錯誤訊息"帳號欄必須輸入"到 errorMsgMap 之內
		// 對應的識別字串為 "accountountEmptyError"
		...
		if (!errorMsgMap.isEmpty()) {
			RequestDispatcher rd = request
					.getRequestDispatcher("/manager/ManagerLogin.jsp");
			rd.forward(request, response);
			return;
		}
		// 4. 進行 Business Logic 運算
		// 將LoginService類別new為物件，存放物件參考的變數為 ls
		ManageLoginService ls = new ManageLoginService();
		// 呼叫 ls物件的 checkIDPassword()，要記得傳入userid與password兩個參數
		// 同時將傳回值放入MemberBean型別的變數mb之內。
		ManageBean mb;
		try {
			mb = ls.checkIDPassword(account, password);
			if (mb != null) {
		...
		if (errorMsgMap.isEmpty()) {
			...
			if (target != null) {
                ...
                response.sendRedirect(
			      response.encodeRedirectURL(contextPath + "/manager/ManageCenter.jsp" ));

			}
			return;
		} else {
			// 如果 errorMsgMap 不是空的，表示有錯誤，交棒給/ch999/Login.jsp
			RequestDispatcher rd = request
					.getRequestDispatcher("/manager/ManagerLogin.jsp");
			rd.forward(request, response);
			return;
